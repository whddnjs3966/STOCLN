...
1. **뉴스 감성 스코어 (News Sentiment) - 가중치 30%**
   * **하이브리드 NLP 엔진:** VADER Sentiment + TextBlob 이중 분석으로 기사의 감성 극성(polarity)과 주관성(subjectivity)을 측정합니다. 두 엔진이 모두 동의해야 긍정/부정으로 판정하여 오탐을 최소화합니다. (stocksight 알고리즘 차용)
   * **AI 요약 보강:** Google Gemini가 기사 본문을 분석하여 3줄 한국어 요약을 생성합니다. 최종 스코어는 NLP(40%) + Gemini(60%) 가중 평균으로 산출됩니다.
   * **기사 본문 스크래핑:** 뉴스 헤드라인 링크를 따라가 전체 기사를 추출(newspaper3k/BeautifulSoup)하여 헤드라인만으로는 불가능한 심층 분석을 수행합니다.
   * Gemini API 미설정 또는 장애 시에도 NLP 단독으로 완전한 분석 결과를 제공합니다.
...
* **Google Gemini API (gemini-2.5-flash):** 뉴스 텍스트 감성 분석 및 핵심 요약 브리핑(3줄 요약) 생성. 무료 티어로 비용 부담 없이 운영 가능.
* **VADER + TextBlob (로컬 NLP):** API 의존 없는 오프라인 감성 분석 엔진. stocksight 프로젝트의 하이브리드 알고리즘을 차용하여 감성 극성·주관성·긍부정 비율을 즉시 산출.
* **newspaper3k / BeautifulSoup:** 뉴스 기사 링크를 따라가 본문 텍스트를 추출하여 NLP 분석 정확도를 향상.
...
* **LLM 환각(Hallucination) 통제:** AI가 존재하지 않는 이슈를 지어내는 것을 막기 위해, 시스템 프롬프트에 "반드시 수집된 기사 텍스트 내에서만 근거를 찾고, 정보가 부족하면 0점(중립) 처리할 것"을 강제합니다. 추가로 NLP 엔진이 독립적으로 검증하여 AI 스코어와의 괴리를 감지합니다.
...
**Phase 1: 백엔드 엔진 & MVP (Must-Have)** ✅ 완료
* Python/FastAPI 기반 비동기 데이터 파이프라인(OpenDart, Naver, yfinance 연동) 구축.
* VADER + TextBlob 하이브리드 NLP 감성 분석 엔진 구현 (stocksight 알고리즘 차용).
* Google Gemini API를 활용한 AI 요약 브리핑 및 NLP-AI 하이브리드 스코어링.
* 뉴스 기사 본문 스크래핑(newspaper3k) 연동으로 분석 정확도 향상.
* 4-Factor 스코어링 알고리즘 로직 완성 및 상세 지표(PER/PBR/ROE/MA/MACD/RSI) 반환.
* Supabase 데이터베이스 스키마 설계.

**Phase 2: 프론트엔드 UI & 연동 (Should-Have)** ✅ 완료
* Next.js 프로젝트 세팅 및 Vercel 배포 연결.
* React Three Fiber를 활용한 3D 구체(Sphere) 스코어 게이지 및 인터랙티브 레이더 차트 구현.
* 검색 커맨드 UI 및 API 연동을 통한 결과 대시보드 출력.
* Score Detail: 기술적/펀더멘털 지표 상세 표시 (한국어 설명 포함).
* Sentiment Detail: NLP 감성 분석 상세 (극성, 주관성, 긍부정 비율 시각화).
